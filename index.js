...
function PDFImage(pdfFilePath, options) {
  ...
  this.adjoinImage = options.adjoinImage || false;
...
PDFImage.prototype = {
  ...
  constructCombineCommandForFile: function (imagePaths) {
    return util.format(
      this.adjoinImage ? "%s %s -adjoin \"%s\"" : "%s -append %s \"%s\"",
      this.useGM ? "gm convert" : "convert",
      imagePaths.join(' '),
  ...
  convertFile: function (pages) {
    var pdfImage = this;
    pages = (pages && Array.isArray(pages)) ? pages : [];
        ...
        var convertPromise = new Promise(function (resolve, reject) {
          ...
          pages.forEach(function (page) {
            pdfImage.convertPage(page).then(function (imagePath) {
              ...
              if (imagePaths.length === pages.length) {
